...
function Climate({
	...
	return (
		<div id="Climate">
			...
			<div className="delete" onClick={() => deleteClimate(index)}>
				{" "}
				<X size={14} />
				{" "}
			</div>
			...
			<div className="item-content">
				...
				<div>
					<p>
						Presion:
						<span>{ current?.pressure ?? "NA" }</span>
					</p>
					<p>
						Temperatura:
						<span>{ current?.temperature ?? "NA" }</span>
					</p>
					<p>
						Humedad:
						<span>{ current?.humidity ?? "NA" }</span>
					</p>
					<p>
						Viento:
						<span>{ current?.wind_speed ?? "NA" }</span>
					</p>
					<p>
						Visibilidad:
						<span>{ current?.visibility ?? "NA" }</span>
					</p>
					<p>
						Descripcion:
						<span>{ current?.weather_descriptions?.join(",") ?? "NA" }</span>
					</p>
				</div>
			</div>
